...
events.onPlayerInteractBlock(function(event as PlayerInteractBlockEvent) {
  ...
  if (!isNull(heldItem)) {
    ...
    event.player.sendChat("- 该点击功能已被禁用，以避免AE2出现Bug");
    event.player.sendChat("");
    event.player.sendChat("- 如果你正在尝试配置P2P，请使用合成配方");
    event.player.sendChat("");
    event.player.sendChat("- 手持ME接口时，会无法打开总线或终端界面，请切换到其他快捷栏槽位");
    event.player.sendChat("");
    event.player.sendChat("- 如果你想把面板形式的接口和P2P放在同一根线缆上，请先放置接口");
    event.cancel();
  }
});
